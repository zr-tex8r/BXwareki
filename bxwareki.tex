...
\begin{itemize}
\item \TeX{} format: \LaTeX, plain {\TeX}.
...
\item Counter |warekiyear|\Means
  ...
  \Note Counters are available only on {\LaTeX}.
  On plain {\TeX}, you can use the command |\thewarekiyear|
  (which expands to numerals) instead.
  \Note Unlike ordinary counters,
  the assignment to |warekiyear| by |\warekisetdate| is \emph{local}.
  Moreover, manual assignment to this counter is \emph{not} supported.
...
\item |\warekicustomdate{|\Meta{option}|}|\Means
  ...
  On other engines (and plain {\TeX} format)
  the command simply falls back to |\warekidate|.
  Users can use |\WarekiIfCustomDateAvailable| to know
  whether the command really works.
\end{itemize}
...
\begin{itemize}
\item On p(\La){\TeX}, up(\La){\TeX} and Ap(\La){\TeX}:
  Nothing is inserted,
  since the engine can automatically insert shibuaki spaces.
\item On Lua(\La){\TeX} and \bxtexlogo{Xe}(\La){\TeX}:
  Nothing is inserted,
...
\item On (\La){\TeX} and pdf(\La){\TeX}:
  A tie (|~|) is inserted,
  on the assumption that the \Pkg{CJK} package
  (or its alternative)
  is employed and |\CJKtilde| is in effect.
\end{itemize}
...
On engines that support |\warekicustomdate|,
you can use the following commands:
\begin{itemize}
\item |\WarekiJxKansuji{|\Meta{counter}|}|\Means
  Prints the counter value using kansuji-by-reading
  with ``ten's multiple'' kanji characters
  (as with |J| options of |\warekicustomdate|).
  Only valid for numbers less than 100.
\item |\WarekiMonthName{|\Meta{counter}|}|\Means
  Prints the \emph{imyo} name of the month.
  Only valid for numbers in range from 1 to 12.
\end{itemize}

%-------------------
\subsection{Checks for availability}

\begin{itemize}
\item |\WarekiIfAvailable{|\Meta{true}|}{|\Meta{false}|}|\Means
  Tests if the package can really write kanji characters.
